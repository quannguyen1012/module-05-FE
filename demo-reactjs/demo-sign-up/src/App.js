...
export default function App() {
  ...
  function handleChange(event) {
    ...
    if(event.target.name === "password"){
      if(form.confirmPassword && form.confirmPassword.value){
        ...
        ? "" : MASSAGE_ERROR[event.target.name];
      }else{
        error = REGEX[event.target.name].test(event.target.value) ? "" : MASSAGE_ERROR[event.target.name];
      }
    }else if (event.target.name === "confirmPassword"){
      error = event.target.value === form.password.value ? "" : MASSAGE_ERROR[event.target.name];
    }else{
      error = REGEX[event.target.name].test(event.target.value) ? "" : MASSAGE_ERROR[event.target.name];
    }
    setForm({
  ...
  function handleSubmit() {
    ...
    alert (  isFilled && !isError ? "Sign in success!!!" : "Please fill out all the fields!!!");
  }
